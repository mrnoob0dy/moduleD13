...
class CommentCreateView(LoginRequiredMixin, CreateView):
    ...
    def form_valid(self, form):
        ...
        send_mail(
            subject=f'У вас новый отклик на объявление!',
            message=f'Здравствуйте, {comment.post.author}, ! На ваше объявление оставили отклик!' f'Посмотреть отклик: http://127.0.0.1:8000/',
...
def confirm_comment(request, **kwargs):
    if request.user.is_authenticated:
        ...
        comment = Comment.objects.get(id=comment.id)
        send_mail(
            subject=f'Форум MMORPG: Ваш отклик принят!',
            message=f'Здравствуйте, {comment.author}, Автор объявления {comment.post.title} принял Ваш отклик!'f'Зайдите в личный кабинет:http://127.0.0.1:8000/posts/comments',
            from_email='testza1@yandex.ru',
            recipient_list=[comment.post.author.email, ],
        )
        return HttpResponseRedirect(reverse('index'))
...
class EmailVereficationView(TemplateView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        email_verefications = EmailVerification.objects.filter(user=user, code=code)
        if email_verefications.exists() and not email_verefications.first().is_expired():
