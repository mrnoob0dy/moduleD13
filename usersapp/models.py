...
class EmailVerification(models.Model):
    ...
    def send_verefication_email(self):
        link = reverse('usersapp:email_verefication', kwargs={'email': self.user.email, 'code': self.code})
        verefication_link = f'{settings.DOMAIN_NAME}{link}'
        subject = f'Подтверждение учетной записи для {self.user.username}'
        message = 'Для подтверждения учетной записил для {} перейдите по ссылке {}'.format(self.user.email, verefication_link)
        send_mail(
            subject=subject,
            message=message,
